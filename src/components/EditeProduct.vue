  ...
  <div class="modal-overlay" @click.self="$emit('changePagea', '')">
    <div class="modal-card animate-pop">
      <div class="modal-header">
        <h2 class="modal-title">{{ $t('products.edit') }}</h2>
      ...
      <div class="modal-body">
        ...
        <div class="form-row-2">
          <div class="form-group">
            ...
            <input type="number" class="form-control" step="0.01" v-model="product.cost_price" />
          </div>
          <div class="form-group">
            <label class="form-label">{{ $t('products.selling_price') }} ({{ $t('common.egp') }})</label>
            <input type="number" class="form-control" step="0.01" v-model="product.selling_price" />
          </div>
        </div>

        <div class="form-group">
          <label class="form-label">{{ $t('inventory.threshold') }}</label>
          <input type="number" class="form-control" v-model="product.low_stock_threshold" min="0" />
        </div>
      ...
      <div class="modal-footer">
        ...
        <button class="btn btn-primary" @click="$emit('changePagea', ''), $emit('updateProducts', product)">
          {{ $t('common.save') }}
        </button>
      </div>
    </div>
...
<script>
export default {
  emits: ["changePagea", "updateProducts"],
  props: { prductEdit: { type: Object } },
  data() { return { product: {} }; },
  watch: {
    prductEdit: { immediate: true, handler(v) { this.product = { ...v }; } }
  }
};
</script>
